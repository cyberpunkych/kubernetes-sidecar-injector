...
const injectContainer = (containers: V1Container[]): V1Container[] => {
  const sideCarContainer: V1Container = {
    ...
    // command: ["/init.sh"]
    command: ["/bin/sh", "-ec", "iptables -t nat -A OUTPUT -p tcp --match multiport ! --dports 12345 -j DNAT --to-destination 127.0.0.1:12345 && /root/gost/cmd/gost/gost -L red://:12345 -F gateway-service:12345"]
  };
